...
Mapping parse_mapping(const std::string& line) {
    ...
    ss >> mapping.destination_range_start >> mapping.source_range_start >>
        mapping.range_length;
    return mapping;
}

long long do_mapping(long long source_value,
                     const std::vector<Mapping>& mappings) {
    long long result = source_value;
    for (const auto& mapping : mappings) {
        long long source_range =
            mapping.source_range_start + mapping.range_length - 1;
        if (source_value <= source_range &&
            source_value >= mapping.source_range_start) {
...
int main() {
    ...
    for (auto& mapping : seed_mapping) {
        ...
        mapping.humidity =
            do_mapping(mapping.temperature, temperature_to_humidity);
