...
void updateMinimum(const std::string& color, int count, size_t& min_red,
                   size_t& min_blue, size_t& min_green) {
...
int minimumPower(std::vector<std::string> rounds) {
    ...
    for (std::string round : rounds) {
        ...
        while (comma_pos != std::string::npos) {
            ...
            std::string color =
                round.substr(space_pos + 1, comma_pos - (space_pos + 1));
...
int main() {
    ...
    while (std::getline(input_file, current_line)) {
        ...
        if (game_pos != std::string::npos) {
            ...
            int game_id = std::stoi(
                current_line.substr(game_pos + 5, colon_pos - (game_pos + 5)));
